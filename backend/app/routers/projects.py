...
async def list_projects(
    ...
    """获取项目列表（分页）。"""
    items, total = await project_service.get_projects(db, page=page, page_size=page_size, status=status)
...
async def delete_project(
    ...
    if not ok:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="项目不存在或已删除")
